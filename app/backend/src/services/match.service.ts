...
class MatchService {
  ...
  static async matchValidation(match: IMatch): Promise<IService<IServiceError> | void> {
    ...
    if (error?.details[0].type) {
      return { statusCode: 'Unauthorized', payload: { message: error.details[0].message } };
    }
  ...
  static async create(match: IMatch): Promise<IService<IMatch | IServiceError>> {
    ...
    if (!id) {
      return { statusCode: 'BadRequest', payload: { message: 'Match not created' } };
    }
    return { statusCode: 'Created', payload: { id, ...match } };
  }

  static async finish(id: number): Promise<IService<IMatch | IServiceError>> {
    const match = await MatchModel.getById(id);
    if (!match) {
      return { statusCode: 'NotFound', payload: { message: 'Match not found' } };
    }
    if (!match.inProgress) {
      return { statusCode: 'BadRequest', payload: { message: 'Match already finished' } };
    }
    match.inProgress = false;
    const updated = await MatchModel.update(match);
    if (!updated) {
      return { statusCode: 'BadRequest', payload: { message: 'Match not updated' } };
    }
    return { statusCode: 'OK', payload: match };
  }
  ...
  static async update(id: number, match: IMatch): Promise<IService<IMatch | IServiceError>> {
    ...
    return { statusCode: 'OK', payload: newMatch };
  }
}
